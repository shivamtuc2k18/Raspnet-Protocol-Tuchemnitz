...
void SendBuffer()
{
	if(sendLock != 1) ///Check if the  sendlock is set.
	{
        ...
        ///Check to see if all the bits of a particular Byte is sent.
			...
			if(send_counter==8 && byteCounter != TotalByte ){ /// if all the bytes are not send then it will reset the bit                                                           counter and make increment in byte counter
			    send_counter = 0;
				byteCounter++;	
			}

				if(byteCounter == TotalByte){ /// if all the bytes are recieved then it will set the lock as 1.
